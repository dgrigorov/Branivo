...
- Публичен Insurer Registry — агрегатор на лицензирани застрахователи (КФН регистър, SEO вход, публичен API) (Epic 23)
...
### 12. Публичен Insurer Registry (Phase 1 / Epic 23)

> FR91–FR95 са добавени след финализиране на PRD v1 (2026-03-26).
> Номерирани след FR90 за да не нарушават съществуващата схема.

- **FR91:** Системата агрегира автоматично (BullMQ седмичен cron) публичния регистър на лицензираните застрахователи от КФН (fsc.bg) и обогатява всеки запис с данни от официалния сайт на застрахователя (лого, описание, контакти) чрез Cheerio/Puppeteer scraping
- **FR92:** Системата използва Claude Haiku API за структуриране на суровите scraped данни в унифициран застрахователен профил (ai_summary, продукти, категории)
- **FR93:** Системата изчислява `sentiment_score` (0–100) за всеки застраховател въз основа на споменавания в публични БГ форуми (forums.bg, automedia.bg); минимум 5 споменавания за валиден score; при по-малко — `null`
- **FR94:** Системата излага публичен REST API без автентикация: `GET /api/v1/public/insurers`, `GET /api/v1/public/insurers/:code`, `GET /api/v1/public/insurers/ranked`; всеки отговор съдържа задължителен disclaimer: "Информацията е агрегирана от публични източници и не представлява застрахователен съвет"
- **FR95:** Публичният API кешира отговорите в Redis (TTL 1 час) и прилага rate limiting (60 req/min/IP) без JWT; `insurer_crawl_raw` таблицата съхранява суровите scraped данни за debugging и ре-процесиране
