...
## Bash команди — правило за `cd`

**НИКОГА** не пускай `cd` като самостоятелна команда — хаби токени без полза.

Преди всяка `cd` команда трябва да знаеш в коя директория се намираш. За целта **добавяй `ls` като последна стъпка към командата непосредствено преди `cd`** — така проверката на директорията и следващата `cd` са в два отделни, но свързани реда:
...
# ПРАВИЛНО — ls се долепя към предишната команда, cd се комбинира с реалната:
git status && ls
cd branivo-api && npm run dev
```

- `ls` се добавя в края на командата ПРЕДИ `cd` — не като самостоятелна команда
- `cd` ВИНАГИ се комбинира с реалната команда: `cd branivo-api && npm run lint`
- При multi-step: `cd dir && cmd1 && cmd2`
...
**ЗАДЪЛЖИТЕЛНО**: Когато се изпълни `/bmad-bmm-code-review` skill:

1. Изпълни code review-то докрай
2. **Автоматично** активирай **Option 1** — оправи ВСИЧКИ намерени проблеми с предложените решения
3. Оправяй проблеми с **всички** severity нива: **High, Medium и Low** (не пропускай Low)
4. След като оправиш всичко, докладвай накратко какво е поправено

> Не питай дали да оправиш — просто го направи. Не пропускай Low severity проблеми.
