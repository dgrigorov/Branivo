...
Данните се съхраняват за срока на полицата + 5 години (КФН изискване) или по-малко ако законодателството позволява.`;
  ...
  private async seedTos(): Promise<void> {
    const content = `# Общи Условия — Demo Broker

**Последна актуализация:** 2026-04-05

## 1. Предмет на договора
Настоящите Общи Условия уреждат отношенията между Demo Broker и крайните клиенти при ползване на платформата за застрахователни услуги Branivo.

## 2. Предоставяни услуги
Demo Broker предоставя: (1) онлайн сравнение на застрахователни продукти; (2) сключване на застрахователни договори; (3) управление на полици и подновявания; (4) уведомления за изтичащи полици.

## 3. Права и задължения на клиента
Клиентът се задължава да предоставя вярна и пълна информация при сключване на застрахователен договор. Предоставянето на неверни данни освобождава застрахователя от задълженията му по полицата. Клиентът е отговорен за опазването на данните за достъп до своя акаунт.

## 4. Плащания и такси
Всички плащания се обработват чрез Stripe. Таксите са посочени изрично при всяка конкретна оферта преди финализиране на поръчката. Demo Broker не съхранява данни за платежни карти — те се обработват директно от Stripe.

## 5. Отговорност
Demo Broker действа като застрахователен посредник, лицензиран от КФН. Отговорността за изплащане на застрахователни обезщетения е на съответния застраховател, не на брокера. Demo Broker не носи отговорност за решения на застрахователя.

## 6. Изменение на условията
Demo Broker може да изменя настоящите Общи Условия. При всяка промяна клиентите ще бъдат уведомени и ще се изисква ново изрично приемане на актуализираните условия преди продължаване на ползването на платформата.

## 7. Приложимо право и юрисдикция
Настоящите Общи Условия се подчиняват на законодателството на Република България. Компетентен съд за разрешаване на спорове е Районен съд — гр. София.`;

    await this.dataSource.query(
      `INSERT INTO tenant_tos_versions
         (id, tenant_id, version, content, language, is_published, published_at, created_by)
       VALUES
         (gen_random_uuid(), $1, 1, $2, 'bg', true, NOW(), NULL)
       ON CONFLICT (tenant_id, version, language) DO NOTHING`,
    ...
    this.logger.log('ToS seeded.');
  }

  private async seedCookiePolicy(): Promise<void> {
    const content = `# Политика за бисквитките — Demo Broker

**Последна актуализация:** 2026-04-06

## Какво са бисквитките

Бисквитките (cookies) са малки текстови файлове, които се съхраняват на Вашето устройство при ползване на нашето приложение. Те ни помагат да предоставяме по-добро потребителско изживяване и да подобряваме услугите си.

## Необходими бисквитки (винаги активни)

Тези бисквитки са необходими за работата на приложението и не могат да бъдат изключени. Те включват: управление на сесии, сигурност на автентикацията, запомняне на настройките на тенанта.

## Аналитични бисквитки (по избор)

Тези бисквитки ни помагат да разбираме как потребителите използват приложението. Информацията е анонимизирана и се използва само за подобряване на функционалността. Включват: статистика за използване, проследяване на грешки (Sentry), производителност на зареждане.

## Маркетингови бисквитки (по избор)

Тези бисквитки се използват за показване на персонализирани предложения и застрахователни продукти, съобразени с Вашите нужди. Включват: проследяване на интереси, персонализация на офертите, ремаркетинг.

## Функционални бисквитки (по избор)

Тези бисквитки запомнят Вашите предпочитания, за да осигурят по-добро изживяване при следващо посещение. Включват: запомняне на езикови предпочитания, последно разглеждани МПС, предпочитан начин на плащане.

## Как да управлявате предпочитанията си

Можете да промените своите настройки за бисквитки по всяко време чрез: Настройки → Настройки на бисквитките. Необходимите бисквитки не могат да бъдат изключени, тъй като са задължителни за работата на приложението.

## Свържете се с нас

При въпроси относно нашата политика за бисквитки: support@demo.com`;

    await this.dataSource.query(
      `INSERT INTO tenant_cookie_policies
         (id, tenant_id, version, content, language, is_published, published_at, created_by)
       VALUES
         (gen_random_uuid(), $1, 1, $2, 'bg', true, NOW(), NULL)
       ON CONFLICT (tenant_id, version, language) DO NOTHING`,
